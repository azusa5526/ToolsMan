...
public class SingleArticleAir extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_single_article_air);

        mDatabase = FirebaseDatabase.getInstance().getReference().child("ArticleAir");

        mPost_key = getIntent().getExtras().getString("article_id");
        //Toast.makeText(SingleArticleAir.this, post_key, Toast.LENGTH_LONG).show();
        ...
        mCommentDB = FirebaseDatabase.getInstance().getReference().child("ArticleAir").child(mPost_key).child("CommentAir");   //存留言的資料庫
    ...
    protected void onStart() {
        ...
        FirebaseRecyclerAdapter<SingleCommentAir, BlogViewHolder> firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<SingleCommentAir, BlogViewHolder>(
                SingleCommentAir.class,
                R.layout.activity_single_comment_air,
    ...
    public void addCommentList() {
        ...
        mCommentDB.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (comment.matches(""))
                {
                    Toast toast = Toast.makeText(SingleArticleAir.this, "請輸入回覆", Toast.LENGTH_SHORT);
                    toast.show();
                }
                else{
                    newPost.child("Comment").setValue(comment);
                    Toast toast = Toast.makeText(SingleArticleAir.this, "回覆成功", Toast.LENGTH_SHORT);
                    toast.show();
                    CommentText = comment;
                    Comment.setText("");
                }
            ...
            @Override
            public void onCancelled(DatabaseError databaseError) {
                Toast toast = Toast.makeText(SingleArticleAir.this, "回覆失敗", Toast.LENGTH_SHORT);
                toast.show();
            }
        });
    }
