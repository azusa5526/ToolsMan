...
public class SingleArticleElec extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mCommentDB = FirebaseDatabase.getInstance().getReference().child("ArticleElec").child(mPost_key).child("CommentElec");   //存留言的資料庫
    ...
    protected void onStart() {
        ...
        FirebaseRecyclerAdapter<SingleCommentElec, BlogViewHolder> firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<SingleCommentElec, BlogViewHolder>(
                SingleCommentElec.class,
                R.layout.activity_single_comment_elec,
                BlogViewHolder.class,
                mCommentDB
        ) {
            @Override
            protected void populateViewHolder(BlogViewHolder viewHolder, SingleCommentElec model, int position) {
                viewHolder.setComment(model.getComment());
                viewHolder.mView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Toast toast = Toast.makeText(SingleArticleElec.this, "選擇師傅", Toast.LENGTH_SHORT);
                        toast.show();
    ...
    public void addCommentList() {
        ...
        mCommentDB.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (comment.matches(""))
                {
                    Toast toast = Toast.makeText(SingleArticleElec.this, "請輸入回覆", Toast.LENGTH_SHORT);
                    toast.show();
                }
                else{
            ...
            @Override
            public void onCancelled(DatabaseError databaseError) {
                Toast toast = Toast.makeText(SingleArticleElec.this, "回覆失敗", Toast.LENGTH_SHORT);
                toast.show();
            }
        });
    }
